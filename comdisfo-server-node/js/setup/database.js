...
function m2db(mid){
    ...
    fields.forEach(function(f, idx){
        if(f.column && f.column!='id' && f.type!=='formula' && !fieldsAttr[f.column]){
            ...
            if(['c_date','u_date','c_uid','u_uid','nb_comments','nb_votings'].indexOf(f.column)<0){
    ...
    if("comments" != tableName && config.wComments){
        fs.push('nb_comments integer DEFAULT 0');
    }

    // - tracking votings.
    if("comments" != tableName && config.wVoting){
        fs.push('nb_votings integer DEFAULT 0');
    }

    // subCollecs - as json columns
    if("comments" != tableName && subCollecs){
...
if(sqlFile){
    ...
    fs.writeFile('comdisfo-db-schema-'+fId+'.sql', sql, function(err){
        if (err){
            throw err;
        }
    })
    fs.writeFile('comdisfo-db-z-data-'+fId+'.sql', sqlData, function(err){
        if (err){
            throw err;
        }
    })  
