...
export default React.createClass({
	...
	render() {
		...
		if(m){
			...
			const ico = icon ? <img className="cdfor-many-icon" src={'/pix/'+icon}/> : null,
			...
			function cell(d, f, idx){
				...
				if(idx===0){
					...
					return <td key={idx}><div className="cdfo-color-box" id={f.id} 
			...
			return (
				<div data-entity={e} style={{width: '100%'}}>
					...
					<div className="comdisfo cdfor-many-list">
						{body}
						<Pagination 
							count={data.length} 
							fullCount={fullCount} 
